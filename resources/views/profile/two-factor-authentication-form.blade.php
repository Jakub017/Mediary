    ...
    <x-slot name="content">
            ...
            @if ($this->enabled)
                @if ($showingConfirmation)
                    <h3 class="text-base font-medium text-gray-900">{{ __('Zakończ konfigurację uwierzytelniania dwuskładnikowego.') }}</h3>
                @else
                    <h3 class="text-base text-green-600 font-medium ">{{ __('Włączono uwierzytelnianie dwuskładnikowe.') }}</h3>
                @endif
            @else
                <h3 class="text-base font-medium text-red-600 ">{{ __('Nie włączono uwierzytelniania dwuskładnikowego.') }}</h3>
            @endif
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                ...
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        {{ __('Klucz konfigarcyjny') }}: {{ decrypt($this->user->two_factor_secret) }}
                    </p>
                </div>
